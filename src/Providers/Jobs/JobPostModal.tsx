...
const JobPostModal = ({ caregiver }: any) => {
  const { userData, contactData, refreshData }: any = useAuthContext();
  ...
  useEffect(() => {
    if (userData) {
      ...
      if (contactData) {
        setValue("telephone", contactData.tel);
      }
    }
  }, [userData, setValue]);
  ...
  const handleFormSubmit = async (data: any) => {
    ...
    try {
      ...
      const profile = await privateApi.post(
