...
function LikeComponent({
  ...
  const fetchLikeStatus = async () => {
    try {
      setLoading(true);
      const res = await privateApi.get(`/v1/forum/like/${type}/${threadID}/${contactData.userID}`
      );
  ...
  const { mutate: likeItem } = useMutation({
    mutationFn: async () => {
      ...
      const { data } = await privateApi.post(`/api/v1/forum/like/${type}/${threadID}`,
        payload
      );
      return data;
    },
  ...
  const { mutate: removeLike } = useMutation({
    mutationFn: async () => {
      ...
      const { data } = await privateApi.post(`/api/v1/forum/like/${type}/${threadID}/remove`,
        payload
      );
      return data;
    },
