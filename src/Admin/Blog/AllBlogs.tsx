 ...
 const {userData} = useAuthContext();
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        const response = await privateApi.get(`/api/v1/blogs/author/${userData.userID}`
        );
  ...
  const createNewBlog = async () => {
    ...
    try {
      const response = await privateApi.post("/api/v1/blogs/create",
        {
          title: "Untitled Blog",
          category: "",
          excerpt: "",
  ...
  const deleteBlog = async (blogId: string) => {
    ...
    try {
      await axios.delete(`http://https://jrp7pe2xhj.us-east-1.awsapprunner.com/api/v1/blogs/delete/${blogId}`);
