              ...
              >
                ...
                <div className="p-6 border-b border-gray-100 bg-gray-50 flex flex-col sm:flex-row justify-between items-start sm:items-center">
                  <div className="flex items-center">
                    <Avatar>
                      <AvatarImage src={ref.profileImage} alt={ref.providerName || ""} />
                      <AvatarFallback>{ref.providerName?.slice(0,2) || "P?"}</AvatarFallback>
                    </Avatar>
                    <div className="ml-4">
                      ...
                      <p className="text-sm text-gray-500 mt-1">
                        Referred on{" "}
                        {ref.referredAt
                          ? format(new Date(ref.referredAt), "MMM d, yyyy, h:mm a")
                          : "N/A"}
                      </p>
                    </div>
                  </div>
                ...
                <div className="p-6 relative">
                  ...
                  <div className="space-y-8 ml-2">
                    {milestonesConfig.map((msCfg) => {
                      const ms = ref.milestones[msCfg.key as keyof typeof ref.milestones];
                      ...
                      return (
                        <div key={msCfg.key} className="flex items-start group">
                          ...
                          <div className="ml-6 flex-1 flex justify-between items-start sm:items-center">
                            <div>
                              <h4 className={`font-medium ${rewarded ? "text-green-700" : "text-gray-900"}`}>
                                {msCfg.title}
                              </h4>
                              <p className="text-sm text-gray-500 mt-1">
                                {rewarded
                                  ? `Earned $${ms.amount} • ${format(new Date(ms.rewardedAt!), "MMM d, yyyy")}`
                                  : `${ms.amount} USD upon ${msCfg.title.toLowerCase()}`}
                              </p>
                            </div>
                            <span className={`font-semibold ${rewarded ? "text-green-600" : "text-gray-400"}`}>
                              ${ms.amount}
                            </span>
                          </div>
                        </div>
                ...
                <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex justify-between items-center">
                  ...
                  >
                    ...
                    <div>
                      ...
                      <span className="block text-sm text-gray-500">Total Earned</span>
                    </div>
                  </motion.div>
        ...
        <section className="space-y-6">
          ...
          <div className="flex justify-center items-center space-x-4 mt-6">
            ...
            <span className="text-sm text-gray-600">
              Page {currentPage} of {Math.ceil(crowdPosts.length / postsPerPage)}
            </span>
            <Button
              size="sm"
              disabled={currentPage === Math.ceil(crowdPosts.length / postsPerPage)}
              onClick={() =>
                setCurrentPage((p) =>
