...
function ApplyNow({ jobID, job }: ApplyNowProps) {
  ...
  const providerName =
    `${job?.provider?.fname ?? ""} ${job?.provider?.lname ?? ""}`.trim();
    ...
    onSuccess: async (data) => {
      ...
      try {
        await axios.post(
          ...
          {
            body: `New Application for ${job?.title ?? "a job"} by ${userData?.fname ?? ""} ${userData?.lname ?? ""}
    ...
    <div>
      <div className="flex gap-2">
        ...
        >
          <Bookmark
            size={18}
            className={`${isFavorite ? "text-purple-700 fill-current" : "text-gray-600"}`}
