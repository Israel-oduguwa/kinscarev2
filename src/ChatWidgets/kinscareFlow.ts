/* KinsCare Conversation Logic (FSM) */
...
export type SessionData = {
  work_preference?: "direct_patient_care" | "non_patient_facing" | "administrative";
...
export type FlowState = {
  step: StepId;
  session: SessionData;
  _awaiting?: "years_of_experience" | "preferred_state" | "educational_background" | "strengths";
...
// ——— Public API ———
export function createFlow(opts?: { userSignedIn?: boolean }): { state: FlowState; ui: UIFrame } {
...
export function restart(_: FlowState | undefined): { state: FlowState; ui: UIFrame } {
  // Full reset of session, per your request
...
export function exitFlow(state: FlowState): { state: FlowState; ui: UIFrame } {
  ...
  return {
    state,
    ui: { messages: [bot("Thanks for chatting with KinsCare today. If you need anything else, I’m here.")], allowFreeText: false },
  };
}
...
export async function handleQuickReply(
  ...
  if (key === "trigger_signup") {
    ...
    return {
      state,
      ui: { messages: [bot("Great! We’ll use your info to personalize matches."), botWithButtons("", [restartBtn(), exitBtn()])], allowFreeText: false },
    };
  }
  ...
  switch (state.step) {
    ...
    case "become_known_ask_track": {
      ...
      if (key === "track_nursing") return goto(state, "become_known_nursing_menu");
      break;
    }
...
export async function handleText(
  ...
  switch (state.step) {
    case "nursing_years_ask": {
      ...
      if (!Number.isFinite(n) || n < 0) {
        return say(`Please enter a valid number of years (e.g., 0, 1, 2).`, true);
      }
    ...
    case "nursing_zip_ask": {
      ...
      return goto(state, "nursing_reco");
    }
  ...
  return {
    state,
    ui: { messages: [user(clean)], allowFreeText: allowFreeTextFor(state.step) },
  };
}

// ——— Transitions + rendering ———
function goto(state: FlowState, next: StepId): { state: FlowState; ui: UIFrame } {
  state.step = next;
  return { state, ui: render(state) };
}
function allowFreeTextFor(step: StepId): boolean {
  return ["nursing_years_ask", "nursing_zip_ask", "become_unknown_edu", "become_unknown_strengths"].includes(step);
}

function render(state: FlowState, opts?: { overrideMessage?: string; quick?: QuickReply[] }): UIFrame {
  ...
  switch (state.step) {
    ...
    case "hire_menu": {
      ...
      push(botWithButtons("What would you like to do next?", [
    ...
    case "hire_jumpstart": {
      push(bullets([
        "We do the work; 2–3 vetted matches in ~3 days.",
        "Save time",
      ]));
      push(bot("Pricing: One-time $200, no hidden charges."));
      push(botWithButtons("", [
        { id: "open_jumpstart", label: "Get Matched by KinsCare ↗", href: "https://www.kinscare.org/jumpstart-hiring/apply" },
        restartBtn(), exitBtn(),
      ]));
      return { messages, allowFreeText: false };
    }
    ...
    case "hire_post": {
      ...
      push(botWithButtons("", [
        { id: "open_post", label: "Find your next caregiver ↗", href: "https://www.kinscare.org/post-job" },
        restartBtn(), exitBtn(),
      ]));
      return { messages, allowFreeText: false };
    }
    ...
    case "hire_browse": {
      ...
      push(botWithButtons("", [
        { id: "open_browse", label: "Browse Caregivers ↗", href: "https://www.kinscare.org/caregivers?shifts=Full+time&licenses=HCA" },
        restartBtn(), exitBtn(),
      ]));
      return { messages, allowFreeText: false };
    }
    ...
    case "caregiver_greeting": {
      push(bot("We help you find jobs, update your profile, and get career support."));
      push(botWithButtons("", [
        { id: "open_jobs", label: "Find Jobs ↗", href: "https://www.kinscare.org/find-jobs" },
        restartBtn(), exitBtn(),
      ]));
      return { messages, allowFreeText: false };
    }
    ...
    case "become_known_ask_track": {
      push(bot("Do you know what career in nursing or allied healthcare you want to pursue?"));
      push(botWithButtons("", [{ id: "know_yes", label: "Yes" }, { id: "know_no", label: "No" }]));
      return { messages, allowFreeText: false };
    }
    ...
    case "become_known_allied_q2": {
      push(bot("Do you prefer helping people recover physically or communicate better (Yes), or do you prefer analyzing medical data (No)?"));
      push(botWithButtons("", [
    ...
    case "become_known_allied_reco": {
      ...
      push(summary("Recommended path (Allied Healthcare)", { career_path, program, recommendation }));
    ...
    case "become_known_nursing_menu": {
      ...
      push(botWithButtons("", [
        { id: "LPN", label: "LPN" }, { id: "RN", label: "RN" }, { id: "BSN", label: "BSN" },
        { id: "MSN", label: "MSN" }, { id: "ARNP", label: "ARNP" }, { id: "DNP", label: "DNP" },
      ]));
      return { messages, allowFreeText: false };
    }
    ...
    case "nursing_prereq_note": {
      ...
      push(botWithButtons("Continue?", [{ id: "nursing_continue", label: "Continue" }]));
      return { messages, allowFreeText: false };
    }

    case "nursing_license_ask": {
      push(bot("Do you currently hold an active license?"));
      push(botWithButtons("", [{ id: "has_cna", label: "CNA" }, { id: "no_license", label: "I have no license" }]));
      return { messages, allowFreeText: false };
    }

    case "nursing_years_ask": {
      push(bot("How many years of experience as a CNA do you have? (enter a number)"));
    ...
    case "nursing_reco": {
      ...
      state.session.recommendation = state._selectedProgram;
      state.session.career_path = "known";
      push(summary(`Top ${state._selectedProgram} programs for you`, {
    ...
    case "become_unknown_edu": {
      push(bot("Which best describes your highest education so far? (e.g., High school, Diploma, Bachelor’s, Master’s)"));
      return { messages, allowFreeText: true };
    }

    case "become_unknown_pref": {
      push(bot("Do you prefer direct patient care, behind-the-scenes work, or administrative tasks?"));
      push(botWithButtons("", [
    ...
    case "become_unknown_strengths": {
      push(bot("What are your strengths? (e.g., teamwork, communication, compassion, problem-solving, attention to detail, organization, technical skills, empathy)"));
      return { messages, allowFreeText: true };
    }

    case "become_unknown_env": {
      push(bot("Are you comfortable with fast-paced or high-stress environments?"));
      push(botWithButtons("", [
    ...
    case "unknown_reco": {
      const { career_path, program, recommendation } = state.session;
      push(summary("Recommended paths for you", { career_path, program, recommendation }));
      push(signupGate(state)); // terminal
      return { messages, allowFreeText: false };
    }
  ...
  if (opts?.overrideMessage) {
    return { messages: [botWithButtons(opts.overrideMessage, opts.quick ?? [])], allowFreeText: false };
  }
  return { messages, allowFreeText: allowFreeTextFor(state.step) };
}

// ——— Reco stubs ———
async function alliedRecommend(state: FlowState) {
...
async function unknownRecommend(state: FlowState) {
  const res = await recommend_trigger({
...
export async function recommend_trigger(input: Record<string, any>): Promise<{ career_path: string; program: string; recommendation: string; }> {
  if (input.context === "allied_healthcare_known") {
    if (input.work_preference === "direct_patient_care") {
      if (input.set_help_people === "rehab_or_communication") {
        return { career_path: "Allied Healthcare", program: "Physical Therapy Aide / SLPA", recommendation: "Hands-on rehab/communication roles fit your preferences. Explore PTA/SLPA certificates near you." };
      }
      return { career_path: "Allied Healthcare", program: "Diagnostic Imaging Tech", recommendation: "Blend patient interaction with technical analysis in imaging programs." };
    } else {
      return { career_path: "Allied Healthcare", program: "Health Info Mgmt / Med Lab Tech", recommendation: "Non-patient-facing options in data, quality, or lab analysis." };
    }
  }
  const pref = input.work_preference;
  if (pref === "direct_patient_care") {
    return { career_path: "Patient Care", program: "CNA → LPN/RN Pathway", recommendation: "Start with CNA to gain clinical exposure, then bridge to LPN/RN." };
  }
  if (pref === "administrative") {
    return { career_path: "Healthcare Administration", program: "Medical Office Admin / HSM", recommendation: "Use organization & communication strengths toward admin leadership." };
  }
  return { career_path: "Healthcare Technology", program: "Health Informatics / Imaging Tech", recommendation: "Combine technical skills with healthcare impact in informatics or imaging." };
}

// ——— Helpers (incl. say + signupGate) ———
function id(prefix: string) { return `${prefix}_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`; }
function bot(text: string): ChatMsg { return { id: id("b"), from: "bot", text, time: "now" }; }
function user(text: string): ChatMsg { return { id: id("u"), from: "user", text, time: "now" }; }
function bullets(lines: string[]): ChatMsg { return bot("• " + lines.join("\n• ")); }
function botWithButtons(text: string, buttons: QuickReply[]): ChatMsg { return { id: id("b"), from: "bot", text, time: "now", quickReplies: buttons }; }
function summary(title: string, fields: Record<string, any>): ChatMsg {
  const rows = Object.entries(fields).filter(([, v]) => v != null && String(v).trim() !== "").map(([k, v]) => `• ${capitalize(k)}: ${v}`);
  return bot(`${title}\n${rows.join("\n")}`);
}
function capitalize(x: string) { return x.slice(0, 1).toUpperCase() + x.slice(1).replace(/_/g, " "); }
function exitBtn(): QuickReply { return { id: "exit", label: "Exit chat" }; }
function restartBtn(): QuickReply { return { id: "restart", label: "Restart conversation" }; }
function isNursing(x: string) { return ["LPN", "RN", "BSN", "MSN", "ARNP", "DNP"].includes(x); }
...
function say(text: string, allowFreeText = false): { state?: FlowState; ui: UIFrame } {
  return { ui: { messages: [bot(text)], allowFreeText } } as any;
}
function signupGate(state: FlowState): ChatMsg {
  if (state.session.user_signed_in) {
    return botWithButtons("You're signed in. Ready to proceed?", [restartBtn(), exitBtn()]);
  }
  return botWithButtons("Sign up to see your matches. We’ll prefill your details to save time.", [
