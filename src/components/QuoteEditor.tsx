...
function QuoteEditor({
  ...
  const { quotedContent, setQuotedContent } = useQuote(); // Get quoted content and ID
  ...
  //   useEffect(() => {
  //     if (quotedContent.content && editor) {
  //       editor.commands.setContent(quotedContent.content + "\n\n");
  //       setQuotedContent({ id: "", content: "" }); // Clear after use
  //     }
  //   }, [quotedContent, editor, setQuotedContent]);
  ...
  //   console.log(quotedContent.id);
  useEffect(() => {
    if (quotedContent.content && quotedContent.id === postID && editor) {
      //   console.log("index of the post =", postID, "and", quotedContent.id);
  ...
  return (
    <>
      ...
      <div className="border prose prose-h3:my-2 prose-blockquote:my-2 lg:prose-lg md:prose-md sm:prose-sm discussion-content max-w-[100%] border-gray-300 rounded-md">
